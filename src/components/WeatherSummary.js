...
export default function WeatherSummary({weatherSummary}) {
	...
	return (
		<>
			<h3 className="mb-3">Your week summary</h3>
						...
						<span className="d-flex align-items-center">
							...
							<div className="d-flex flex-column">
								{weatherSummary?.weather_types?.map(weatherType => {
									return (<p key={weatherType} className="mb-0">{capitalize(weatherType.replaceAll('_', ' '))}</p>)
							})}
							</div>
						</span>
					</div>
